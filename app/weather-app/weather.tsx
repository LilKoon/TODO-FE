...
import { Search, Wind, Droplets, Eye, Thermometer, Sun, Cloud, Navigation, Settings, User, Clock } from "lucide-react";
            ...
            <main className="flex flex-1 p-4 gap-4 overflow-hidden bg-[#101A23] border-b border-gray-800">
                ...
                <div className="w-[70%] flex flex-col gap-4 h-full">
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <p className="text-white text-2xl font-bold">{weatherData?.name || "Loading..."}</p>
                            <p className="text-gray-400 text-sm font-medium">{new Date().toDateString()}</p>
                        </div>
                        <button
                        ...
                        ><Navigation className="w-3 h-3" /> Current Location</button>
                    </div>
                    ...
                    <div className="text-white bg-gradient-to-br from-[#1E293B] to-[#111827] p-6 rounded-2xl flex justify-between items-center relative shadow-lg h-48">
                        <div className="z-10 pl-2">
                            ...
                            <div className="text-sm text-gray-400">
                                Feels Like: <span className="text-white font-bold">{weatherData ? Math.round(weatherData.main.feels_like) : "--"}°</span>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col">
                        <p className="text-md font-bold text-gray-300 mb-3">Current Details</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 h-full text-white">
                            ...
                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Wind className="w-4 h-4" /> Wind</div>
                                <div className="text-xl font-bold">{weatherData?.wind?.speed} <span className="text-xs text-gray-400">{unit === 'metric' ? 'm/s' : 'mph'}</span></div>
                                <div className="text-[10px] text-gray-400">Deg: {weatherData?.wind?.deg}°</div>
                            </div>

                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Thermometer className="w-4 h-4" /> Pressure</div>
                                <div className="text-xl font-bold">{weatherData?.main?.pressure} <span className="text-xs text-gray-400">hPa</span></div>
                                <div className="text-[10px] text-gray-400">Sea Level</div>
                            </div>

                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Sun className="w-4 h-4" /> Sun & Time</div>
                                <div className="text-[10px] text-gray-300 flex justify-between w-full border-b border-gray-700 pb-1">
                                    <span>Sunrise</span> <span>{weatherData ? formatTime(weatherData.sys.sunrise) : "--"}</span>
                                </div>
                                <div className="text-[10px] text-gray-300 flex justify-between w-full pt-1">
                                    <span>Sunset</span> <span>{weatherData ? formatTime(weatherData.sys.sunset) : "--"}</span>
                                </div>
                            </div>

                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Droplets className="w-4 h-4" /> Humidity</div>
                                <div className="text-xl font-bold">{weatherData?.main?.humidity} <span className="text-xs text-gray-400">%</span></div>
                                <div className="text-[10px] text-gray-400">Dew point: N/A</div>
                            </div>

                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Eye className="w-4 h-4" /> Visibility</div>
                                <div className="text-xl font-bold">{weatherData ? (weatherData.visibility / 1000).toFixed(1) : "--"} <span className="text-xs text-gray-400">km</span></div>
                                <div className="text-[10px] text-gray-400">Clear view</div>
                            </div>

                            <div className="bg-[#1E293B] p-3 rounded-xl flex flex-col justify-between h-28">
                                <div className="flex items-center gap-2 text-gray-400 text-xs font-medium"><Cloud className="w-4 h-4" /> Cloudiness</div>
                                <div className="text-xl font-bold">{weatherData?.clouds?.all} <span className="text-xs text-gray-400">%</span></div>
                                <div className="w-full bg-gray-700 h-1.5 rounded-full mt-2">
                                    <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${weatherData?.clouds?.all}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="w-[30%] bg-[#1E293B] rounded-2xl p-5 h-full">
                    ...
                    <div className="flex flex-col gap-1 h-full overflow-y-auto pr-1">
                        {[0, 1, 2, 3, 4, 5, 6].map((day, i) => (
                            <div key={i} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-0 hover:bg-[#2b384b] px-2 rounded transition-colors text-xs">
                                <span className="text-gray-400 w-10">{new Date(Date.now() + day * 86400000).toLocaleDateString("en-US", { weekday: 'short' })}</span>
                                <div className="flex items-center gap-1">
                                    <img src={`http://openweathermap.org/img/wn/${i % 2 === 0 ? '01d' : '02d'}.png`} className="w-6 h-6" alt="icon" />
                                    <span className="text-white font-bold">{i % 2 === 0 ? 'Sunny' : 'Rain'}</span>
                                </div>
            ...
            <footer className="h-48 shrink-0 flex px-4 pb-4 gap-4 bg-[#0B131E]">
                <div className="w-[70%] flex flex-col justify-center">
                    <div className="flex justify-between items-center mb-2 px-1">
                        ...
                        <span className="text-xs text-blue-400 cursor-pointer hover:underline">Next 24 Hours</span>
                    </div>

                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide text-white">
                        {hourlyData.map((item, index) => (
                            <div key={index} className={`flex flex-col items-center justify-center min-w-[80px] h-28 rounded-2xl border border-[#202B3B] transition-colors cursor-pointer ${item.active ? 'bg-[#308DED] text-white' : 'bg-[#1E293B] hover:bg-[#253246]'}`}>
                                <span className={`text-xs font-bold mb-1 ${!item.active && 'text-gray-400'}`}>{item.time}</span>
                                <img src={`http://openweathermap.org/img/wn/${item.icon}.png`} className="w-10 h-10" alt="icon" />
                ...
                <div className="w-[30%] h-full pt-6">
                    <div className="h-full bg-[#2C333D] rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group cursor-pointer border-2 border-transparent hover:border-gray-500 transition-all">
                        <p className="text-xs font-bold text-gray-400 group-hover:text-white">View Radar Map</p>
                    </div>
                </div>
